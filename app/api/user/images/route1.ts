...
export async function GET(request: NextRequest) {
  ...
  const imageIds = images.rows.filter(i => i.visibility === 'selected').map(i => i.id);
  let accessMap: Record<number, number[]> = {};

  if (imageIds.length > 0) {
    const access = await sql`
      SELECT ia.image_id, ia.allowed_user_id, u.name
      FROM image_access ia
      JOIN users u ON ia.allowed_user_id = u.id
      WHERE ia.image_id = ANY(${imageIds}::int[])`;

    access.rows.forEach((r: any) => {
      if (!accessMap[r.image_id]) accessMap[r.image_id] = [];
      accessMap[r.image_id].push(r.allowed_user_id);
    });
  }
